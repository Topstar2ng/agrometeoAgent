const { createApp, ref, onMounted, onUnmounted, computed } = Vue;
        ...
        createApp({
            setup() {
                ...
                // Function to call your Express backend AI agent route - FIXED VERSION
                ...
                const generateFarmAIInsight = async () => {
                    ...
                    try {
                        ...
                        // Handle API rate limiting scenario with a specific user-facing message
                        ...
                        // This will directly read the JSON object returned from Express
                        const data = await response.json();
                        aiReport.value = data; 
                ...
                // Quick-fill presets to simulate quick action shortcuts
                ...
                const sendChatMessage = async () => {
                    ...
                    // Auto-scroll layout to latest user bubble
                ...
                const activeAlerts = computed(() => {
                    ...
                    // If no telemetry has loaded yet, return an empty slate
                    ...
                    // 1. Frost / Freezing Threat Matrix
                    if (temp <= 5) {
                        list.push({
                            id: 'frost',
                    ...
                    // 2. Extreme Heat & Evaporation Threat Matrix
                    if (temp >= 33) {
                        list.push({
                            id: 'heat',
                    ...
                    // 3. Flash Flood / Intense Precipitation Threat Matrix
                    if (rain >= 15) {
                        list.push({
                            id: 'flood',
                    ...
                    // 4. Critical Atmospheric Drought Matrix
                ...
                onMounted(() => {
                    ...
                    // Your existing detectLocation function
                    detectLiveLocation();
                });
                
                // Now onUnmounted is defined and will work
                onUnmounted(() => {
                ...
                return {
                    activeTab, locationMode, liveCoords, manualCoords, selectedLocationIndex, activeCoordinates, activeLatLonString, activeLocationName, activeView, weatherData, loading, error,
                    showAuthModal, authMode, isAuthenticated, currentUser, authForm, authMessage, executeAuthSubmit, closeAuthWorkspace, logoutSession, switchEngineView, refreshTelemetry, formatDayName, getForecastIconClass, detectLiveLocation, applyManualCoordinates, generateFarmAIInsight, aiReport, generatingReport, cropSelection, generateFarmAIInsight, parseMarkdownToHtml,isChatOpen, chatMessage, chatMessages, isChatTyping,
                    toggleChat, sendChatMessage, setChatPreset, activeAlerts, scrollChatBottom, isOnline, networkQuality, connectionType, showOfflineBanner
                };
            }
        }).mount('#app');